...
from IO import write_list_to_file,load_list_from_file
...
def calibrate(R,power_low,power_high,graduation=10,samples=15,drive_turn="drive",write=True):
    ...
    if write==True:
        if drive_turn=="drive":
            write_list_to_file("ben/drive_calibration.txt",data)
            print "writing driving constants"
        elif drive_turn=="turn":
            write_list_to_file("ben/turn_calibration.txt",data)
...
def load_calibrations(files=("drive_calibration.txt","turn_calibration.txt")):
